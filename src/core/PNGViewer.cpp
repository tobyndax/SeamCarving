...
unsigned char * PNGViewer::getData()
{
	return image;
}

unsigned char *PNGViewer::getDataSingleChannel()
{
...
void PNGViewer::setData(std::shared_ptr<std::vector<unsigned char>> data, int width, int height)
{
	w = width == 0 ? w : width;
	h = height == 0 ? h : height;
	size_t n = width * height * 4;
	if (image) delete image;
	image = new unsigned char[4 * n]();
	std::vector<unsigned char>* input = data.get();
	for (int i = 0; i < w*h; ++i) {
		image[i * 4 + 0] = input->at(i);
		image[i * 4 + 1] = input->at(i);
		image[i * 4 + 2] = input->at(i);
		image[i * 4 + 3] = 255; // no alpha for this one
	}
}
